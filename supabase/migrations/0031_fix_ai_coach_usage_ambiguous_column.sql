-- check_and_increment_ai_coach_usage (0029/0030) returns table(..., request_count int, ...),
-- which makes plpgsql treat "request_count" as an implicit output variable, ambiguous
-- against the identically-named column on the actual increment step:
--   update ai_coach_usage set request_count = request_count + 1 ...
-- -- "column reference request_count is ambiguous" (42702), caught live via the browser
-- console against the deployed function. Qualifying the right-hand side with the table
-- name resolves it without renaming anything client-facing.
